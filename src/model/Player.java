...
public class Player extends MapObject {
	...
	private ArrayList<BolaFogo> fireBalls;
	...
	public Player(TileMap tm) {
		...
		fireCost = 200;
		fireBallDamage = 5;
		fireBalls = new ArrayList<BolaFogo>();

		scratchDamage = 8;
		scratchRange = 40;

		try {
			coracao = ImageIO.read(getClass().getResourceAsStream("/HUD/heart.png"));
	...
	public void checkMoedasColisao(ArrayList<Moeda> moedas) {
	...
	public void checkAttack(ArrayList<Inimigo> inimigos) {
	...
	public void update() {
		...
		if(firing && currentAction != FIREBALL) {
			if(fire > fireCost) {
				fire -= fireCost;
				BolaFogo fb = new BolaFogo(tileMap, facingRight);
				fb.setPosition(x, y);
				fireBalls.add(fb);
			}
		}
	...
	public void draw(Graphics2D g) {
		...
		for(int j = 0; j < life; j++) {
			g.drawImage(coracao, 140 + (15*j), 10, 15, 15, null);
		}
	...
	public void setLife(int live) {this.life = live;}
